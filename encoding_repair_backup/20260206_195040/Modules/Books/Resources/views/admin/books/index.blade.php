...
<div class="mb-6 bg-white p-4 rounded-lg shadow">
    <form method="GET" action="{{ route('admin.books.index') }}" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="بحث في الكتب..." class="flex-1 px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-accent">
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-4 md:px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">رابط الشراء</th>
                ...
                @forelse($books as $book)
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-4 md:px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center hidden md:table-cell">
                            <a href="{{ $book->external_url }}" target="_blank" rel="nofollow sponsored noopener" class="text-blue-600 hover:underline truncate max-w-[150px] inline-block">
                                {{ Str::limit($book->external_url, 30) }}
                            </a>
                        </td>
                        <td class="px-4 md:px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center hidden md:table-cell">
                            ...
                            <div class="flex items-center gap-2 justify-center flex-wrap">
                                ...
                                <form action="{{ route('admin.books.destroy', $book) }}" method="POST" class="inline">
                                    ...
                                    <button type="submit" class="js-confirm inline-flex items-center px-3 py-1.5 bg-red-50 text-red-700 rounded-md hover:bg-red-100 hover:text-red-800 font-medium text-xs transition-colors duration-200"
                                            data-confirm-message="هل أنت متأكد من حذف هذا الكتاب؟">
                                        <svg class="w-3.5 h-3.5 ml-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
