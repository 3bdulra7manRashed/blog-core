...
@section('title', 'ط§ظ„ظ…ظ‚ط§ظ„ط§طھ')

@section('content')
<div class="mb-6 flex items-center justify-between">
    <h1 class="text-3xl font-serif font-bold text-brand-primary">ط§ظ„ظ…ظ‚ط§ظ„ط§طھ</h1>
...
<div class="mb-6 bg-white p-4 rounded-lg shadow">
    <form method="GET" action="{{ route('admin.posts.index') }}" class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="ط¨ط­ط« ظپظٹ ط§ظ„ظ…ظ‚ط§ظ„ط§طھ..." class="flex-1 px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-accent">
        <select name="status" class="py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-accent">
            ...
            <option value="published" {{ request('status') === 'published' ? 'selected' : '' }}>ظ…ظ†ط´ظˆط±</option>
        </select>
        <button type="submit" class="px-4 py-2 bg-gray-700 text-white rounded-md hover:bg-gray-800 whitespace-nowrap">طھطµظپظٹط©</button>
        @if(request()->hasAny(['search', 'status']))
            <a href="{{ route('admin.posts.index') }}" class="px-4 py-2 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400 text-center whitespace-nowrap">ظ…ط³ط­</a>
        @endif
    </form>
</div>
    ...
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    <th class="px-4 md:px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider min-w-[200px]">ط§ظ„ط¹ظ†ظˆط§ظ†</th>
                    <th class="px-4 md:px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">ط§ظ„ظƒط§طھط¨</th>
                    ...
                    <th class="px-4 md:px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">طھط§ط±ظٹط® ط§ظ„ظ†ط´ط±</th>
                ...
                @forelse($posts as $post)
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-4 md:px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center hidden md:table-cell">
                            {{ $post->author?->name ?? 'ظ…ط³طھط®ط¯ظ… ظ…ط­ط°ظˆظپ' }}
                        </td>
                            ...
                            <div class="flex items-center gap-2 justify-center flex-wrap">
                                ...
                                <form action="{{ route('admin.posts.destroy', $post) }}" method="POST" class="inline">
                                    ...
                                    <button type="submit" class="js-confirm inline-flex items-center px-3 py-1.5 bg-red-50 text-red-700 rounded-md hover:bg-red-100 hover:text-red-800 font-medium text-xs transition-colors duration-200"
                                            data-confirm-message="ظ‡ظ„ ط£ظ†طھ ظ…طھط£ظƒط¯ ظ…ظ† ط­ط°ظپ ظ‡ط°ط§ ط§ظ„ظ…ظ‚ط§ظ„طں">
                                        <svg class="w-3.5 h-3.5 ml-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
