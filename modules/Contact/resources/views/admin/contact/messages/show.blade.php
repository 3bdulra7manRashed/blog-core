...
@section('content')
    <div class="max-w-4xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                ...
                <form action="{{ route('admin.messages.toggle-read', $message) }}" method="POST">
                    ...
                    @method('PATCH')
                    <button type="submit"
                        class="inline-flex items-center px-4 py-2 {{ $message->is_read ? 'bg-blue-50 text-blue-600 hover:bg-blue-100' : 'bg-gray-100 text-gray-600 hover:bg-gray-200' }} rounded-lg transition-colors">
                        @if($message->is_read)
                            <svg class="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="bg-gradient-to-l from-brand-accent/10 to-transparent border-b border-gray-100 px-6 py-5">
                <div class="flex items-start justify-between">
                    <div class="flex items-center">
                        <div
                            class="flex-shrink-0 w-14 h-14 bg-gradient-to-br from-brand-accent to-amber-600 rounded-full flex items-center justify-center text-white font-bold text-xl shadow-lg">
                            {{ mb_substr($message->name, 0, 1) }}
                        </div>
                        <div class="mr-4">
                            <h2 class="text-xl font-bold text-[#1F3A6E]">{{ $message->name }}</h2>
                            <div class="flex items-center gap-4 mt-1">
                                <a href="mailto:{{ $message->email }}"
                                    class="text-sm text-brand-accent hover:underline flex items-center" dir="ltr">
                                ...
                                @if($message->phone)
                                    <a href="tel:{{ $message->phone }}"
                                        class="text-sm text-gray-600 hover:text-brand-accent flex items-center" dir="ltr">
                                        <svg class="w-4 h-4 ml-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <h3 class="text-lg font-semibold text-[#1F3A6E] mb-4">الرد السريع</h3>
            <div class="flex items-center gap-3">
                <a href="mailto:{{ $message->email }}?subject=رد: رسالة من موقع {{ config('app.name') }}&body=%0A%0A---%0Aفي {{ $message->created_at->format('Y/m/d') }}، كتب {{ $message->name }}:%0A{{ urlencode($message->message) }}"
                    class="inline-flex items-center px-5 py-2.5 bg-brand-accent text-white font-medium rounded-lg hover:bg-opacity-90 transition-colors hover:text-white hover:bg-[#2f5c8f] ">
                    <svg class="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z">
                        </path>
