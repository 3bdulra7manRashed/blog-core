...
<form action="{{ route('admin.settings.landing.update') }}" method="POST" enctype="multipart/form-data">
    ...
    <div class="flex flex-col lg:flex-row gap-6">
        ...
        <div class="w-full lg:w-1/3 space-y-6">
            ...
            <!-- Mobile Hero Image -->
            <div class="bg-white p-4 rounded-lg shadow">
                <h3 class="text-lg font-bold text-[#1F3A6E] mb-4">صورة الموبايل (Hero Mobile)</h3>
                ...
                @if($settings->hero_mobile_image)
                    <div class="mb-4">
                        <img 
                            ...
                            class="w-full h-40 object-cover rounded-lg border"
                        >
                        <p class="text-sm text-gray-500 mt-1">صورة الموبايل الحالية</p>
                    </div>
                @endif
                ...
                <div class="mb-4">
                    ...
                    <input 
                        ...
                        accept="image/*"
                        class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm @error('hero_mobile_image') border-red-500 @enderror"
                    >
                    @error('hero_mobile_image')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                    <p class="text-xs text-gray-500 mt-1">اختياري: يعرض للزوار باستخدام الموبايل (Fallback: صورة الخلفية الأفتراضية)</p>
                </div>

                <!-- Or Use URL -->
                <div class="mb-4">
                    <label for="hero_mobile_image_url" class="block text-sm font-medium text-gray-700 mb-1">أو رابط صورة خارجية</label>
                    <input 
                        type="text" 
                        name="hero_mobile_image_url" 
                        id="hero_mobile_image_url" 
                        placeholder="https://example.com/mobile-image.jpg"
                        class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm @error('hero_mobile_image_url') border-red-500 @enderror"
                    >
