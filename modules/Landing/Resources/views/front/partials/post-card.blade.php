...
@php
    ...
    $defaultRoute = $contentType === 'khutbah' ? 'khutab.show' : 'post.show';
    $targetRoute = $itemRoute ?? $defaultRoute;
    $ctaLabel = $contentType === 'khutbah' ? 'اقرأ الخطبة' : 'اقرأ المقال';
@endphp
<article
    ...
    @if($post->featured_image_url)
        <a href="{{ route($targetRoute, $post->slug) }}" class="block aspect-video overflow-hidden">
            <img src="{{ $post->featured_image_url }}" alt="{{ $post->title }}"
                class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
        </a>
    @else
        <a href="{{ route($targetRoute, $post->slug) }}"
