...
@section('content')
        ...
        <form action="{{ route('admin.media.library.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="flex-1">
                <input type="file" name="file"
                    accept=".jpeg,.jpg,.png,.gif,.webp,.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.zip,.rar" required class="block w-full text-sm text-gray-900 border rounded-lg cursor-pointer bg-gray-50 focus:outline-none 
            ...
            <button type="submit"
                class="flex items-center justify-center px-4 py-2 bg-brand-accent text-white rounded-lg hover:bg-teal-700 transition-colors shadow-sm hover:shadow-md whitespace-nowrap">
    ...
    @if(request()->has('search') || $media->count() > 0)
        <div class="mb-4">
            <form method="GET" action="{{ route('admin.media.library.index') }}" class="flex items-center gap-3">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="بحث باسم الملف..."
                    class="flex-1 px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-accent text-sm">
                <button type="submit"
                    class="px-4 py-2 bg-brand-primary text-white rounded-md hover:bg-[#0d9488] transition-colors duration-200 text-sm">بحث</button>
                @if(request()->has('search'))
                    <a href="{{ route('admin.media.library.index') }}"
                        class="px-4 py-2 bg-gray-300 text-gray-700 rounded-md hover:bg-gray-400 text-sm">مسح</a>
                @endif
            </form>
        </div>
    @endif
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
        @forelse($media as $item)
            ...
            <div
                class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-all duration-300 group flex flex-col">
                <div
                    class="aspect-square bg-gray-50 relative overflow-hidden border-b border-gray-100 group-hover:border-gray-200">
                    <img src="{{ $imageUrl }}" alt="{{ $item->alt_text ?? $item->filename }}"
                ...
                <div class="p-3 flex flex-col gap-3 flex-1">
                    <div class="flex items-start justify-between gap-2">
                        ...
                        <span
                            class="shrink-0 text-[10px] font-bold text-gray-500 bg-gray-100 border border-gray-200 px-1.5 py-0.5 rounded">
                            {{ number_format($item->size / 1024, 1) }} KB
                        </span>
                    </div>

                    <div class="flex items-center gap-2 mt-auto">
                        <button type="button" onclick="copyToClipboard('{{ $imageUrl }}', this)"
                        ...
                        <form action="{{ route('admin.media.library.destroy', $item->id) }}" method="POST" class="inline-block">
                            ...
                            <button type="submit"
                                ...
                                title="حذف الملف" data-confirm-message="هل أنت متأكد من حذف هذا الملف نهائياً؟">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        @empty
            <div
                class="col-span-full flex flex-col items-center justify-center py-16 bg-gray-50 rounded-lg border-2 border-dashed border-gray-200">
