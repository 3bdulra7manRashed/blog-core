    ...
    <div x-data="{ 
            ...
            <div class="flex items-center gap-3">
                ...
                @if($campaign->isDraft())
                    <a href="{{ route('admin.campaigns.edit', $campaign) }}"
                        class="flex items-center gap-2 px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors bg-white">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="flex flex-col lg:flex-row lg:items-start gap-8">
            ...
            <div class="w-full lg:w-2/3">
                ...
                <!-- Preview Header -->
                <div
                    class="bg-white rounded-t-xl border border-b-0 border-gray-200 px-6 py-4 flex items-center justify-between">
                    <div class="flex items-center gap-3">
            ...
            <div class="w-full lg:w-1/3 lg:sticky lg:top-6 h-fit lg:self-start space-y-5" x-data="{
                ...
                <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                    <h3 class="font-bold text-[#1F3A6E] mb-4 text-sm">خطوات الحملة</h3>
                    ...
                    <div class="relative">
                        ...
                        <div class="flex items-center gap-3 mb-5 relative">
                            <div
                                class="w-6 h-6 rounded-full bg-green-500 flex items-center justify-center flex-shrink-0 z-10 shadow-sm">
                        ...
                        <div class="flex items-center gap-3 mb-5 relative">
                            ...
                            <!-- Active State -->
                            <div x-show="status !== 'sent' && status !== 'sending'"
                                class="w-6 h-6 rounded-full bg-brand-accent flex items-center justify-center flex-shrink-0 z-10 shadow-sm">
                        ...
                        <div class="flex items-center gap-3 relative">
                            ...
                            <div x-show="status !== 'sent'"
                                class="w-6 h-6 rounded-full border-2 border-gray-300 bg-white flex items-center justify-center flex-shrink-0 z-10">
                ...
                <div class="bg-white p-5 rounded-xl shadow-sm border border-gray-100">
                    ...
                    <div class="mt-4 pt-4 border-t border-gray-100">
                        <div class="flex items-center gap-2 text-sm text-gray-600">
                            ...
                            <span class="truncate"
                                title="{{ $campaign->subject }}">{{ Str::limit($campaign->subject, 35) }}</span>
                        </div>
                ...
                <div>
                    <!-- Launch Card -->
                    <div x-show="status === 'draft' || status === 'sending'"
                        class="bg-gradient-to-br from-brand-accent/5 to-blue-50 p-5 rounded-xl border border-brand-accent/20">
                        <h3 class="font-bold text-[#1F3A6E] mb-3 text-sm flex items-center gap-2">
                        ...
                        <button type="button" @click="showModal = true" :disabled="{{ $subscriberCount }} === 0"
                        ...
                        <div class="relative bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden z-10">
                            ...
                            <div class="p-6">
                                <div class="text-center mb-6">
                                    <h3 class="text-xl font-bold text-[#1F3A6E]">هل أنت متأكد؟</h3>
                                    <p class="text-gray-500 mt-2">أنت على وشك إرسال هذه الحملة إلى <span
                                            class="font-bold text-brand-accent">{{ number_format($subscriberCount) }}</span>
                                        مشترك.</p>
                                </div>
                                <div class="grid grid-cols-2 gap-3 items-stretch">
                                    <form action="{{ route('admin.campaigns.send', $campaign) }}" method="POST"
                                        class="h-full" @submit="handleSubmit()">
                                        @csrf
                                        <button type="submit" :disabled="isSending"
                                            class="w-full h-full px-4 py-3 bg-brand-accent text-white rounded-xl hover:bg-brand-accent-hover transition-all font-bold flex items-center justify-center gap-2">
                                            <span x-text="isSending ? 'جارٍ الإرسال...' : 'نعم، أرسل الآن'">نعم، أرسل
                                                الآن</span>
                                        </button>
                                    </form>
                                    <button type="button" @click="showModal = false" :disabled="isSending"
                                        class="w-full h-full px-4 py-3 border-2 border-gray-300 text-gray-700 rounded-xl hover:bg-gray-50 transition-colors font-medium">إلغاء</button>
                                </div>
