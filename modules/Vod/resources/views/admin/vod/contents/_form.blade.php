        ...
        <div class="bg-white p-6 rounded shadow">
            ...
            <div class="@error('description') border border-red-500 rounded @enderror">
                ...
                <textarea name="description" id="description"
                    class="ckeditor">{{ old('description', $content->description ?? '') }}</textarea>
            </div>
            @error('description')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        ...
        <div class="bg-white p-4 rounded shadow">
            ...
            <div class="mb-4">
                ...
                <select name="status" id="status"
                    class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-brand-accent">
                    <option value="draft" {{ old('status', $content->status ?? '') == 'draft' ? 'selected' : '' }}>مسودة
                    </option>
                    <option value="published" {{ old('status', $content->status ?? '') == 'published' ? 'selected' : '' }}>منشور</option>
                    <option value="archived" {{ old('status', $content->status ?? '') == 'archived' ? 'selected' : '' }}>
                        مؤرشف</option>
                </select>
            </div>
            ...
            <div class="pt-4 border-t mt-4">
                <button type="submit"
                    class="w-full px-6 py-2 bg-brand-primary text-white rounded hover:bg-[#0d9488] transition-colors duration-200 text-sm font-medium shadow-sm">
        ...
        <div class="bg-white p-4 rounded shadow">
            ...
            <div class="space-y-2">
                <label class="flex items-center space-x-2 space-x-reverse cursor-pointer hover:bg-gray-50 p-1 rounded">
                    <input type="radio" name="type" value="video" {{ old('type', $content->type ?? 'video') == 'video' ? 'checked' : '' }}
                ...
                <label class="flex items-center space-x-2 space-x-reverse cursor-pointer hover:bg-gray-50 p-1 rounded">
                    <input type="radio" name="type" value="audio" {{ old('type', $content->type ?? '') == 'audio' ? 'checked' : '' }}
                        class="rounded-full border-gray-300 text-brand-accent focus:ring-brand-accent h-4 w-4">
            ...
            <div class="space-y-3">
                <div class="relative border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:border-brand-accent transition-colors cursor-pointer"
                    ...
                    <div id="image-preview"
                        class="{{ (isset($content) && $content->thumbnail_path) ? '' : 'hidden' }} mb-2">
                    ...
                    <div id="upload-placeholder"
                        class="{{ (isset($content) && $content->thumbnail_path) ? 'hidden' : '' }}">
