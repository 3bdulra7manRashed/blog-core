...
@section('content')
    ...
    <div class="mb-6 bg-white p-4 rounded-lg shadow">
        <form method="GET" action="{{ route('admin.vod.contents.index') }}"
            class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            <input type="text" name="search" value="{{ request('search') }}" placeholder="بحث في المحتوى..."
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-4 md:px-6 py-4 min-w-[200px]">
                                ...
                                <div class="flex flex-col items-center text-center gap-y-2 md:block md:text-center">
                                    ...
                                    <a href="{{ $content->route ?? '#' }}" target="_blank"
                                        class="text-sm font-medium text-brand-primary hover:text-brand-accent transition-colors line-clamp-2">
                                        {{ $content->title }}
                                    </a>
                                    <!-- Author Name (Mobile only) -->
                                    <div class="text-sm text-gray-900 md:hidden">{{ $content->author?->name ?? 'مستخدم محذوف' }}
                                    </div>
                            ...
                            <td
                                ...
                                @if($content->type === 'video')
                                    <span
                                        class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-50 text-blue-700">فيديو</span>
                                @else
                                    <span
                                        class="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-purple-50 text-purple-700">صوت</span>
                                @endif
                            </td>
