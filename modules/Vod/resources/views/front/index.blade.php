...
<div class="container mx-auto px-4 py-12 max-w-6xl">
    ...
    <div class="mb-12 flex flex-col items-center justify-center text-center">
        ...
        <p class="mb-8 text-lg text-gray-500">تصفح أحدث {{ $type === 'video' ? 'المقاطع المرئية' : 'المقاطع الصوتية' }} والسلاسل</p>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            @foreach($contents as $item)
                ...
                <div class="group flex flex-col gap-3">
                    {{-- Thumbnail --}}
                    <a href="{{ $route }}" class="block relative aspect-video w-full overflow-hidden rounded-xl bg-gray-100 shadow-sm transition-all duration-300 hover:shadow-md">
                        @if($thumbnail)
                            <img src="{{ Storage::url($thumbnail) }}" alt="{{ $title }}" class="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105">
                        @else
                        ...
                        <div class="absolute inset-0 flex items-center justify-center bg-black/20 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
                            <div class="flex h-12 w-12 items-center justify-center rounded-full bg-white/90 shadow-lg transform scale-90 transition-transform duration-300 group-hover:scale-100">
                        ...
                        {{-- 16:9 Badge --}}
                        <div class="absolute bottom-2 right-2 rounded bg-black/70 px-1.5 py-0.5 text-[10px] font-bold text-white">
                            16:9
                        </div>
                    </a>
                    ...
                    <div class="flex flex-col gap-1">
                        <h3 class="text-lg font-bold leading-snug text-brand-primary line-clamp-2 group-hover:text-brand-accent transition-colors">
    ...
    @else
        <div class="text-center py-20 border border-dashed border-gray-200 rounded-xl bg-gray-50">
            <svg class="w-16 h-16 text-gray-300 mx-auto mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M7 4v16M17 4v16M3 8h4m10 0h4M3 12h18M3 16h4m10 0h4M4 20h16a1 1 0 001-1V5a1 1 0 00-1-1H4a1 1 0 00-1 1v14a1 1 0 001 1z"/></svg>
            <h3 class="text-lg font-bold text-gray-900 mb-1">لا توجد محتويات حالياً</h3>
            <p class="text-gray-500">لم يتم إضافة أي {{ $type === 'video' ? 'فيديوهات' : 'صوتيات' }} بعد.</p>
        </div>
    @endif
</div>
@endsection
