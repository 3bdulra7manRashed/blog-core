    ...
    <div class="container mx-auto px-4 py-12 max-w-6xl">
            ...
            @if(config('features.vod.playlists'))
                <div class="inline-flex rounded-lg bg-gray-100 p-1">
                    <a href="{{ $type === 'video' ? route('videos.index') : route('audios.index') }}"
                        class="rounded-md px-6 py-2 text-sm font-bold transition-all {{ $currentTab !== 'playlists' ? 'bg-brand-accent text-brand-secondary shadow-sm hover:bg-[#3B75B0] hover:text-white' : 'text-gray-500 hover:text-gray-700' }}">
                ...
                @foreach($contents as $item)
                    ...
                    <div class="group flex flex-col gap-3">
                        ...
                        <a href="{{ $route }}"
                            ...
                            {{-- 16:9 Badge --}}
                            <div
                                class="absolute bottom-2 right-2 rounded bg-black/70 px-1.5 py-0.5 text-[10px] font-bold text-white">
                                16:9
                            </div>
                        </a>
        ...
        @else
            <div class="text-center py-20 border border-dashed border-gray-200 rounded-xl bg-gray-50">
                ...
                <h3 class="text-lg font-bold text-gray-900 mb-1">لا يوجد محتوى حالياً</h3>
                <p class="text-gray-500">لم تُضَف أي {{ $type === 'video' ? 'فيديوهات' : 'صوتيات' }} بعد.</p>
            </div>
        @endif
    </div>
@endsection