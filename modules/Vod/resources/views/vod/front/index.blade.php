            ...
            @if(config('features.vod.playlists'))
                <div class="inline-flex rounded-lg bg-gray-100 p-1">
                    <a href="{{ $type === 'video' ? route('videos.index') : route('audios.index') }}"
                        class="rounded-md px-6 py-2.5 text-base font-bold transition-all {{ $currentTab !== 'playlists' ? 'bg-brand-accent text-brand-secondary shadow-sm hover:bg-[var(--brand-accent-hover)] hover:text-white' : 'text-gray-500 hover:text-gray-700' }}">
