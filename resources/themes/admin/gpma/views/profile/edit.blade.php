    ...
    <div class="py-12" dir="rtl" x-data="{ photoPreview: null }">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div class="md:hidden order-1">
                    <div class="bg-white shadow-md rounded-lg p-4 flex items-center gap-4">
                        ...
                        <div class="relative group cursor-pointer flex-shrink-0"
                            ...
                            <div class="w-16 h-16 rounded-full overflow-hidden shadow-inner bg-cover bg-center"
                                x-show="photoPreview" x-cloak :style="'background-image: url(' + photoPreview + ')'"></div>
                            <div
                        ...
                        <div class="flex-1 min-w-0">
                            <h3 class="text-lg font-bold text-[#1F3A6E] truncate">{{ $user->name }}</h3>
                            <p class="text-sm text-gray-500 truncate">{{ $user->email }}</p>
                            @if($user->is_super_admin)
                                <span
                                    class="inline-block mt-1 px-2 py-0.5 bg-purple-100 text-purple-800 text-xs font-semibold rounded-full">مدير
                                    النظام</span>
                            @elseif($user->is_admin)
                                <span
                                    class="inline-block mt-1 px-2 py-0.5 bg-blue-100 text-blue-800 text-xs font-semibold rounded-full">مسؤول</span>
                            @else
                                <span
                                    class="inline-block mt-1 px-2 py-0.5 bg-gray-100 text-gray-800 text-xs font-semibold rounded-full">مستخدم</span>
                            @endif
                        </div>
                    </div>
                </div>

                <!-- Left Column: Forms (Desktop: Right, Mobile: Below Header) -->
                <div class="md:col-span-8 space-y-6">

                    <!-- Consolidated Profile Information Form -->
                    <div class="bg-white shadow-md rounded-lg overflow-hidden">
                        <div class="px-4 md:px-6 py-4 border-b border-gray-100 bg-gray-50">
                            <h3 class="text-lg font-bold text-[#1F3A6E]">تعديل البيانات الشخصية</h3>
                        </div>
                    ...
                    <div class="bg-white shadow-md rounded-lg overflow-hidden">
                        <div class="px-4 md:px-6 py-4 border-b border-gray-100 bg-gray-50">
                            <h3 class="text-lg font-bold text-[#1F3A6E]">تحديث كلمة المرور</h3>
                        </div>
                        <div class="p-4 md:p-6">
                ...
                <!-- Right Column: Profile Card (Desktop: Left Sidebar, Hidden on Mobile) -->
                <div class="hidden md:block md:col-span-4 space-y-6">
                    ...
                    <div class="bg-white shadow-md rounded-lg overflow-hidden md:sticky md:top-4">
                        ...
                        <div class="px-4 md:px-6 py-4 border-b border-gray-200 bg-gray-50">
                            <h3 class="text-lg font-bold text-[#1F3A6E]">معلومات الحساب</h3>
                        </div>
                        ...
                        <div class="px-4 md:px-6 py-6">
                            ...
                            <div class="flex flex-col items-center text-center">
                                ...
                                <div class="relative group cursor-pointer mb-4"
                                    onclick="document.getElementById('photo').click()">
                                    <!-- Current Photo -->
                                    <div class="w-24 h-24 rounded-full overflow-hidden shadow-md border-2 border-gray-200 group-hover:border-indigo-400 transition-all"
                                        x-show="!photoPreview">
                                        <img src="{{ $user->profile_photo_url }}" alt="{{ $user->name }}"
                                            class="w-full h-full object-cover">
                                    </div>
                                    <!-- Preview Photo -->
                                    <div class="w-24 h-24 rounded-full overflow-hidden shadow-md border-2 border-indigo-500 bg-cover bg-center"
                                ...
                                <!-- Name & Email -->
                                <h3 class="text-xl font-bold text-[#1F3A6E]">{{ $user->name }}</h3>
                                <p class="text-sm text-gray-500 mb-3">{{ $user->email }}</p>

                                <!-- Role Badge -->
                                @if($user->is_super_admin)
                                    <span
                                        class="px-3 py-1 bg-purple-100 text-purple-800 text-xs font-semibold rounded-full">مدير
                                        النظام</span>
                                @elseif($user->is_admin)
                                    <span
                                        class="px-3 py-1 bg-blue-100 text-blue-800 text-xs font-semibold rounded-full">مسؤول</span>
                                @else
                                    <span
                                        class="px-3 py-1 bg-gray-100 text-gray-800 text-xs font-semibold rounded-full">مستخدم</span>
                                @endif
                            </div>
