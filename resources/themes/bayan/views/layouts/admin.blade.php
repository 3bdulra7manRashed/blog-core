...
<head>
    ...
    <title>@yield('title', '·ÊÕ… «· Õﬂ„') - {{ config('app.name') }}</title>
...
<body class="bg-gray-100 font-sans overflow-y-scroll" style="scrollbar-gutter: stable;">
    ...
    @if($plainPassword)
        <div class="mb-4 bg-blue-50 border-l-4 border-blue-500 text-blue-700 p-4 rounded-lg shadow-sm" role="alert"
            ...
            <div class="flex items-center">
                ...
                <p class="font-medium">ﬂ·„… «·„—Ê— ÂÌ: <strong>{{ $plainPassword }}</strong> ( ŸÂ— „—… Ê«Õœ… ›ﬁÿ)</p>
            </div>
        </div>
    @endif
    ...
    <div id="site-confirm-modal" aria-hidden="true"
        ...
        <div role="dialog" aria-modal="true" aria-labelledby="site-confirm-title"
            ...
            <h3 id="site-confirm-title" class="text-xl font-semibold text-gray-800 mb-3"> √ﬂÌœ «·≈Ã—«¡</h3>

            <p id="site-confirm-message" class="text-gray-600 mb-4">Â· √‰  „ √ﬂœø</p>
            ...
            <div id="site-confirm-input-wrapper" class="mb-4 hidden">
                <input id="site-confirm-input" type="text" class="w-full border rounded px-3 py-2 text-center"
                    placeholder="«ﬂ » ﬂ·„… «· √ﬂÌœ Â‰«" />
                <p id="site-confirm-help" class="text-sm text-gray-500 mt-2">«ﬂ » <strong>Õ–›</strong> ·· √ﬂÌœ</p>
            ...
            <div class="flex items-center justify-center gap-4" dir="rtl">
                <button id="site-confirm-ok"
                    class="px-6 py-2.5 rounded-lg bg-red-600 text-white font-medium hover:bg-red-700 transition-colors duration-200 shadow-sm hover:shadow-md">Õ–›</button>
                <button id="site-confirm-cancel"
                    class="px-5 py-2.5 rounded-lg border border-gray-300 text-gray-700 font-medium hover:bg-gray-50 hover:border-gray-400 transition-colors duration-200">≈·€«¡</button>
            </div>
        </div>
    </div>
            ...
            <div class="p-6 pt-6 md:pt-6">
                ...
                <nav class="space-y-2">
                    <a href="{{ route('admin.dashboard') }}"
                        class="flex items-center px-4 py-2 rounded-md transition-colors {{ request()->routeIs('admin.dashboard') ? 'bg-brand-accent  text-white hover:text-white hover:bg-teal-800' : 'text-gray-700 hover:bg-gray-100' }}">
                    ...
                    <a href="{{ route('admin.posts.index') }}"
                        class="flex items-center px-4 py-2 rounded-md transition-colors {{ request()->routeIs('admin.posts.*') ? 'bg-brand-accent  text-white hover:text-white hover:bg-teal-800' : 'text-gray-700 hover:bg-gray-100' }}">
                        <svg class="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="{{ route('admin.categories.index') }}"
                        class="flex items-center px-4 py-2 rounded-md transition-colors {{ request()->routeIs('admin.categories.*') ? 'bg-brand-accent  text-white hover:text-white hover:bg-teal-800' : 'text-gray-700 hover:bg-gray-100' }}">
                        <svg class="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="{{ route('admin.tags.index') }}"
                        class="flex items-center px-4 py-2 rounded-md transition-colors {{ request()->routeIs('admin.tags.*') ? 'bg-brand-accent text-white hover:text-white hover:bg-teal-800' : 'text-gray-700 hover:bg-gray-100' }}">
                        <svg class="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <a href="{{ route('admin.media.library.index') }}"
                        class="flex items-center px-4 py-2 rounded-md transition-colors {{ request()->routeIs('admin.media.library.*') ? 'bg-brand-accent text-white hover:text-white hover:bg-teal-800' : 'text-gray-700 hover:bg-gray-100' }}">
                        <svg class="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @if(vod_enabled())
                        @if(config('features.vod.video') || config('features.vod.audio'))
                            <x-feature-link feature="vod" route="admin.vod.contents.index"
                                class="flex items-center px-4 py-2 rounded-md transition-colors {{ request()->routeIs('admin.vod.contents.*') ? 'bg-brand-accent text-white hover:text-white hover:bg-teal-800' : 'text-gray-700 hover:bg-gray-100' }}">
                                <svg class="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        @if(config('features.vod.playlists'))
                            <x-feature-link feature="vod" route="admin.vod.playlists.index"
                                class="flex items-center px-4 py-2 rounded-md transition-colors {{ request()->routeIs('admin.vod.playlists.*') ? 'bg-brand-accent text-white hover:text-white hover:bg-teal-800' : 'text-gray-700 hover:bg-gray-100' }}">
                                <svg class="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <x-feature-link feature="download" route="admin.downloads.index"
                        class="flex items-center px-4 py-2 rounded-md transition-colors {{ request()->routeIs('admin.downloads.*') ? 'bg-brand-accent text-white hover:text-white hover:bg-teal-800' : 'text-gray-700 hover:bg-gray-100' }}">
                        <svg class="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @if(feature('contact'))
                        <x-feature-link feature="contact" route="admin.messages.index"
                            class="flex items-center justify-between px-4 py-2 rounded-md transition-colors {{ request()->routeIs('admin.messages.*') ? 'bg-brand-accent text-white hover:text-white hover:bg-teal-800' : 'text-gray-700 hover:bg-gray-100' }}">
                            <div class="flex items-center">
                                <svg class="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <x-feature-link feature="newsletter" route="admin.campaigns.index"
                        class="flex items-center px-4 py-2 rounded-md transition-colors {{ request()->routeIs('admin.campaigns.*') ? 'bg-brand-accent text-white hover:text-white hover:bg-teal-800' : 'text-gray-700 hover:bg-gray-100' }}">
                    ...
                    <x-feature-link feature="newsletter" route="admin.subscribers.index"
                        class="flex items-center px-4 py-2 rounded-md transition-colors {{ request()->routeIs('admin.subscribers.*') ? 'bg-brand-accent text-white hover:text-white hover:bg-teal-800' : 'text-gray-700 hover:bg-gray-100' }}">
                        <svg class="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @if(feature('manage_admins'))
                        @can('manage-users')
                            <div class="pt-4 border-t border-gray-200 mt-4">
                                <p class="px-4 py-2 text-xs font-semibold text-gray-500 uppercase">≈œ«—… Œ«’…</p>
                    ...
                    <div class="pt-4 border-t border-gray-200 mt-4">
                        ...
                        <a href="{{ route('profile.edit') }}"
                            class="flex items-center px-4 py-2 rounded-md transition-colors {{ request()->routeIs('profile.edit') ? 'bg-brand-accent text-white hover:text-white hover:bg-teal-800' : 'text-gray-700 hover:bg-gray-100' }}">
                            <svg class="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            window.showConfirmModal = function (message, form, expectedText = null, placeholder = null, okText = ' √ﬂÌœ') {
                currentForm = form;
                ...
                if (expectedText && inputWrapper) {
                    ...
                    inputEl.value = '';
                    inputEl.placeholder = placeholder || '«ﬂ » ﬂ·„… «· √ﬂÌœ';
                    if (helpEl) helpEl.innerHTML = `«ﬂ » <strong>${expectedText}</strong> ·· √ﬂÌœ`;
            ...
            function handleConfirm() {
                ...
                if (expectedInput && inputEl) {
                    if (inputEl.value.trim() !== expectedInput) {
                        if (errorEl) {
                            errorEl.textContent = `·„ Ì „ «· √ﬂÌœ »‘ﬂ· ’ÕÌÕ.`;
            ...
            window.confirmDelete = function (form, message = 'Â· √‰  „ √ﬂœ „‰ Õ–› Â–« «·⁄‰’— ‰Â«∆Ì«ø') {
                // Open the modal
                window.showConfirmModal(message, form, null, null, 'Õ–›');
            ...
            document.addEventListener('click', function (e) {
                ...
                const message = confirmBtn.getAttribute('data-confirm-message') || 'Â· √‰  „ √ﬂœ „‰  ‰›Ì– Â–« «·≈Ã—«¡ø';
                ...
                const okText = confirmBtn.getAttribute('data-confirm-ok-text') || ' √ﬂÌœ';
