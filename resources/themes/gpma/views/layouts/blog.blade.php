                ...
                <nav class="hidden md:flex items-center space-x-6 space-x-reverse">
                    ...
                    @if(feature('vod.video'))
                        <a href="{{ route('videos.index') }}"
                            class="text-m font-medium {{ request()->routeIs('videos.*') ? 'text-brand-accent font-bold' : 'text-gray-800' }} hover:text-brand-accent transition-colors">
                            مكتبة الفيديو
                        </a>
                    @endif
                    ...
                    @if(feature('vod.audio'))
                        <a href="{{ route('audios.index') }}"
                            class="text-m font-medium {{ request()->routeIs('audios.*') ? 'text-brand-accent font-bold' : 'text-gray-800' }} hover:text-brand-accent transition-colors">
                            الصوتيات
                        </a>
                    @endif
                    ...
                    @if(feature('books') && ($hasBooks ?? false))
                        <a href="{{ route('books.index') }}"
                            class="text-m font-medium {{ request()->routeIs('books.*') ? 'text-brand-accent font-bold' : 'text-gray-800' }} hover:text-brand-accent transition-colors">
                            الإصدارات
                        </a>
                    @endif
                    ...
                    @if(feature('khutab'))
                        <a href="{{ route('khutab.index') }}"
                            class="text-m font-medium {{ request()->routeIs('khutab.*') ? 'text-brand-accent font-bold' : 'text-gray-800' }} hover:text-brand-accent transition-colors">
                            الخطب
                        </a>
                    @endif

                    <a href="{{ route('about') }}"
                        class="text-m font-medium {{ request()->routeIs('about') ? 'text-brand-accent font-bold' : 'text-gray-800' }} hover:text-brand-accent transition-colors">
                        عني
                    </a>
                    @if(feature('contact') && Route::has('contact'))
                        <a href="{{ route('contact') }}"
                            class="text-m font-medium text-gray-800 hover:text-brand-accent transition-colors">
                            تواصل معي
                        </a>
                    @endif
                </nav>
                ...
                <div class="flex flex-row-reverse items-center gap-5 ml-0 md:mr-auto">
                    ...
                    <div class="relative flex items-center" id="search-container">
                        <form id="search-form" action="{{ route('search') }}" method="GET"
                            ...
                            <button type="button" id="search-toggle"
                                class="text-gray-600 hover:text-brand-primary transition-colors z-10">
                            ...
                            <div id="search-input-wrapper"
                                ...
                                <input type="text" name="q" placeholder="ابحث هنا..."
                                    class="w-48 px-3 py-1 text-sm border-b border-gray-500 focus:border-brand-accent focus:outline-none bg-transparent text-gray-600 placeholder-gray-400 mr-2 text-right dir-rtl">
                            </div>
                        </form>
                    </div>

                    <!-- Sticky Menu Button (Hidden by default, appears on scroll) -->
                    <div id="sticky-menu-btn-container"
                        class="w-0 h-8 overflow-hidden transition-all duration-300 ease-in-out opacity-0 pointer-events-none">
                        <button id="sticky-menu-btn"
                            class="p-1 rounded-full border border-gray-200 hover:border-brand-accent hover:text-brand-accent bg-gray-50 text-gray-600 flex items-center justify-center w-8 h-8 flex-shrink-0">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    @if(config('branding.social.twitter'))
                        <a href="{{ config('branding.social.twitter') }}" target="_blank" rel="noopener noreferrer"
                            class="text-gray-500 hover:text-brand-primary transition-colors" title="X (Twitter)">
                    ...
                    @if(config('branding.social.linkedin'))
                        <a href="{{ config('branding.social.linkedin') }}" target="_blank" rel="noopener noreferrer"
                            class="text-gray-500 hover:text-brand-primary transition-colors" title="LinkedIn">
                            <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                                <path
