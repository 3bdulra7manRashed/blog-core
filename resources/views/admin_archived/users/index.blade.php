...
@section('title', 'ط§ظ„ظ…ط¯ظٹط±ظٹظ†')

@section('content')
<div class="mb-6 flex items-center justify-between flex-wrap gap-4">
    <h1 class="text-3xl font-serif font-bold text-brand-primary">ط§ظ„ظ…ط¯ظٹط±ظٹظ†</h1>
    ...
    <div class="flex items-center gap-3">
        ...
        <x-feature-link feature="manage_admins" route="admin.users.create" class="inline-flex items-center px-4 py-2 bg-brand-accent text-white hover:bg-amber-700 hover:text-white rounded-md hover:bg-opacity-90 transition-colors">
            <svg class="w-5 h-5 ml-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
            </svg>
            <span>ط¥ط¶ط§ظپط© ظ…ط¯ظٹط± ط¬ط¯ظٹط¯</span>
        </x-feature-link>
    </div>
</div>
        ...
        <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-50">
                <tr>
                    ...
                    <th class="px-4 md:px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">ط§ظ„ط¨ط±ظٹط¯ ط§ظ„ط¥ظ„ظƒطھط±ظˆظ†ظٹ</th>
                    <th class="px-4 md:px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell">ط§ظ„ط£ط¯ظˆط§ط±</th>
                    <th class="px-4 md:px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider hidden lg:table-cell">طھط§ط±ظٹط® ط§ظ„طھط³ط¬ظٹظ„</th>
                    <th class="px-4 md:px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">ط§ظ„ط­ط§ظ„ط©</th>
                    ...
                    <tr class="hover:bg-gray-50 transition-colors {{ $user->trashed() ? 'opacity-60 bg-gray-50' : '' }}">
                                        ...
                                        @elseif($user->roles->count() > 0)
                                            @foreach($user->roles as $role)
                                                <span class="inline-block px-2 py-0.5 text-xs font-semibold rounded-full {{ $role->name === 'admin' ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800' }}">
                                                    {{ $role->name === 'admin' ? 'ظ…ط¯ظٹط± ط§ظ„ظ†ط¸ط§ظ…' : ($role->name === 'moderator' ? 'ظ…ط´ط±ظپ' : $role->name) }}
                                                </span>
                                            @endforeach
                                        @endif
                                    </div>
                        ...
                        <td class="px-4 md:px-6 py-4 whitespace-nowrap text-center hidden md:table-cell">
                            ...
                            @elseif($user->roles->count() > 0)
                                ...
                                @endforeach
                            @else
                                <span class="px-2 py-1 text-xs font-semibold rounded-full bg-gray-100 text-gray-800">ط¨ط¯ظˆظ† ط¯ظˆط±</span>
                            @endif
                        </td>
                            ...
                            <div class="flex items-center gap-2 justify-center flex-wrap">
                                @if($user->trashed())
                                    ...
                                    @can('restore', $user)
                                        <form action="{{ route('admin.users.restore', $user->id) }}" method="POST" class="inline">
                                            ...
                                            <button type="submit" class="js-confirm inline-flex items-center px-3 py-1.5 bg-blue-50 text-blue-700 rounded-md hover:bg-blue-100 hover:text-blue-800 font-medium text-xs transition-colors duration-200"
                                                    data-confirm-message="ظ‡ظ„ ط£ظ†طھ ظ…طھط£ظƒط¯ ظ…ظ† ط§ط³طھط¹ط§ط¯ط© ظ‡ط°ط§ ط§ظ„ظ…ط³طھط®ط¯ظ…طں ظ…ظ„ط§ط­ط¸ط©: ط§ظ„ظ…ظ‚ط§ظ„ط§طھ ط³طھط¨ظ‚ظ‰ ظ…ط¹ ط­ط³ط§ط¨ ط§ظ„ظ…ط³طھط®ط¯ظ… ط§ظ„ظ…ط­ط°ظˆظپ.">
                                    ...
                                    @can('forceDelete', $user)
                                        @if($user->id !== 1)
                                            <form action="{{ route('admin.users.forceDelete', $user->id) }}" method="POST" class="inline">
                                                ...
                                                <button type="submit" class="js-confirm inline-flex items-center px-3 py-1.5 bg-red-50 text-red-700 rounded-md hover:bg-red-100 hover:text-red-800 font-medium text-xs transition-colors duration-200"
                                                        data-confirm-message="طھط­ط°ظٹط±: ظ‡ظ„ ط£ظ†طھ ظ…طھط£ظƒط¯ ظ…ظ† ط§ظ„ط­ط°ظپ ط§ظ„ظ†ظ‡ط§ط¦ظٹطں ظ‡ط°ط§ ط§ظ„ط¥ط¬ط±ط§ط، ظ„ط§ ظٹظ…ظƒظ† ط§ظ„طھط±ط§ط¬ط¹ ط¹ظ†ظ‡!"
                                                        data-confirm-input="ط­ط°ظپ"
                                                        data-confirm-input-placeholder="ط§ظƒطھط¨ 'ط­ط°ظپ' ظ„ظ„طھط£ظƒظٹط¯">
                                                    <svg class="w-3.5 h-3.5 ml-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                                                    </svg>
                                                    ط­ط°ظپ ظ†ظ‡ط§ط¦ظٹ
                                        ...
                                        @else
                                            <span class="inline-flex items-center px-3 py-1.5 bg-gray-100 text-gray-400 rounded-md font-medium text-xs">ظ…ط­ظ…ظٹ</span>
                                        @endif
                                    @endcan
                                @else
                                ...
                                @if(!$user->hasRole('admin'))
                                    <form action="{{ route('admin.users.promote', $user) }}" method="POST" class="inline">
                                        ...
                                        <button type="submit" class="js-confirm inline-flex items-center px-3 py-1.5 bg-green-50 text-green-700 rounded-md hover:bg-green-100 hover:text-green-800 font-medium text-xs transition-colors duration-200"
                                                data-confirm-message="ظ‡ظ„ ط£ظ†طھ ظ…طھط£ظƒط¯ ظ…ظ† طھط±ظ‚ظٹط© ظ‡ط°ط§ ط§ظ„ظ…ط³طھط®ط¯ظ… ط¥ظ„ظ‰ ظ…ط´ط±ظپطں">
                                            <svg class="w-3.5 h-3.5 ml-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @else
                                        <form action="{{ route('admin.users.demote', $user) }}" method="POST" class="inline">
                                            ...
                                            <button type="submit" class="js-confirm inline-flex items-center px-3 py-1.5 bg-orange-50 text-orange-700 rounded-md hover:bg-orange-100 hover:text-orange-800 font-medium text-xs transition-colors duration-200"
                                                    data-confirm-message="ظ‡ظ„ ط£ظ†طھ ظ…طھط£ظƒط¯ ظ…ظ† ط¥ط²ط§ظ„ط© ط¯ظˆط± ط§ظ„ظ…ط´ط±ظپطں">
                                                <svg class="w-3.5 h-3.5 ml-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                @can('delete', $user)
                                    @if($user->id !== 1 && $user->id !== auth()->id())
                                        <form action="{{ route('admin.users.destroy', $user) }}" method="POST" class="inline">
                                            ...
                                            @method('DELETE')
                                            <button type="submit" class="js-confirm inline-flex items-center px-3 py-1.5 bg-red-50 text-red-700 rounded-md hover:bg-red-100 hover:text-red-800 font-medium text-xs transition-colors duration-200"
                                                    data-confirm-message="ظ‡ظ„ ط£ظ†طھ ظ…طھط£ظƒط¯ ظ…ظ† ط­ط°ظپ ظ‡ط°ط§ ط§ظ„ظ…ط³طھط®ط¯ظ…طں ط³ظٹطھظ… ظ†ظ‚ظ„ ط¬ظ…ظٹط¹ ظ…ظ‚ط§ظ„ط§طھظ‡ ط¥ظ„ظ‰ ط­ط³ط§ط¨ ط§ظ„ظ…ط³طھط®ط¯ظ… ط§ظ„ظ…ط­ط°ظˆظپ.">
                                                <svg class="w-3.5 h-3.5 ml-1.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                                                </svg>
                                                ط­ط°ظپ
