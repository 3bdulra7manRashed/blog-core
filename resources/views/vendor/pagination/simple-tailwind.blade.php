...
@if ($paginator->hasPages())
    <nav role="navigation" aria-label="Pagination Navigation" class="flex items-center justify-center gap-3" dir="rtl">

        {{-- Previous Button --}}
        @if ($paginator->onFirstPage())
            <span aria-disabled="true"
                class="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-gray-100 text-gray-300 text-sm font-medium cursor-not-allowed">
        ...
        <span
            class="inline-flex items-center justify-center min-w-[2.5rem] h-10 px-3 rounded-full bg-brand-accent text-white font-bold text-sm shadow-lg shadow-brand-accent/30">
            {{ $toArabic($paginator->currentPage()) }}
        </span>
